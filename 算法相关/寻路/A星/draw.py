...
COLOR_BEGAN = 'green'
COLOR_END = 'red'
COLOR_BAD_POINT = 'light sea green'
COLOR_ROUTE = 'orange2'
COLOR_OPEN = 'pale green'
COLOR_CLOSE = 'misty rose'
COLOR_WRITE = 'dark violet'
...
def clean(lPoint):
    pen.color(COLOR_CLEAN)
    drawBG(lPoint)

def write(lPoint, sTxt, bDrawPanel = False):
    if bDrawPanel:
        pen.color(COLOR_CLEAN)
        drawBG(lPoint)
    world_x, world_y = zero[0] + (lPoint[0]+0.1) * g_iBoardSize, zero[1] + (lPoint[1]+0.1) * g_iBoardSize
    pen.color(COLOR_WRITE)
    pen.goto(world_x, world_y)
    pen.pendown()
    pen.write(sTxt, font=('Arial', 10, 'normal'))
    pen.penup()
...
def drawOpenListPoint(lPoint):
    pen.color(COLOR_OPEN)
    drawChess(lPoint)

def drawCloseListPoint(lPoint):
    pen.color(COLOR_CLOSE)
    drawChess(lPoint)
